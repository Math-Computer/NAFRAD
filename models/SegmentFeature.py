...
if __name__ == '__main__':
    inputs = torch.randn(16, 576, 64, 64).to('cuda:7')
    network = DiscriminativeSubNetwork(in_channels=576, out_channels=1).to('cuda:7')
    outputs = network(inputs)

    r = torch.randn_like(outputs).to('cuda:7')
    loss_fn = nn.MSELoss()
    optimizer = torch.optim.Adam(network.parameters(), lr=1e-5)
    loss = loss_fn(outputs, r)
    loss.backward()
