...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        return s_score, ssim_map


if __name__ == '__main__':
    predict = torch.ones(4, 1, 3, 3)
    target = torch.zeros(4, 1, 3, 3)
    loss = focal_loss(predict, target)
    ssimvalue = SSIM()
    f1 = torch.randn(4, 6, 8, 8).to('cuda:7')
    M = nn.Conv2d(6, 6, 3, padding=1).to('cuda:7')
    # f2 = 0.75*f1
    f2 = M(f1)
    s_score, ssim_map = ssimvalue(f1, f2)
    ...
    ssim_mean = torch.mean(ssim_map, dim=1)
    ssim_mean = torch.unsqueeze(ssim_mean, dim=1)
